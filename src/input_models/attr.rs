...
pub fn impl_input_types(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let code = format!(
        r###"impl {struct_name}{{
                ...
                pub async fn parse_http_input<TRequestCredentials: my_http_server::RequestCredentials + Send + Sync + 'static,>(http_route: &my_http_server_controllers::controllers::HttpRoute, ctx: &mut {HTTP_CONTEXT})->Result<Self, {HTTP_FAIL_RESULT}>{{
                    {model_reader}
                }}
        }}"###,
