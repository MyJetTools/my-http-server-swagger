...
pub fn build_action(attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    result.push_str(
        format!("async fn handle_request<TRequestCredentials: my_http_server::RequestCredentials + Send + Sync + 'static,>(&self, http_route: &my_http_server_controllers::controllers::HttpRoute, ctx: &mut {HTTP_CONTEXT}) -> Result<{HTTP_OK_RESULT}, {HTTP_FAIL_RESULT}> {{")
            .as_str(),
    );
